...
dns_cache_ttl_sec: 86400               # DNS 응답 캐시 TTL(초). 0이면 캐시 비활성화
# dns_cache_db: C:/Users/<you>/AppData/Local/FreeGSM/dns-cache.sqlite3
```
...
| `dns_cache_ttl_sec` | `86400` | SQLite DNS 캐시 유지 시간(초). `0`이면 캐시 비활성화 |
| `dns_cache_db` | `%LOCALAPPDATA%\FreeGSM\dns-cache.sqlite3` | DNS 캐시 DB 파일 경로 |
...
set FREEGSM_DNS_CACHE_DB=C:\Users\<you>\AppData\Local\FreeGSM\dns-cache.sqlite3
```
...
- **DNS 캐시**: 같은 DNS 질의가 반복될 때는 SQLite 캐시에 저장된 응답을 먼저
  사용합니다. DNS 메시지의 앞 2바이트 질의 ID는 요청마다 달라질 수 있으므로,
  캐시 키는 질의 ID를 제외한 DNS 본문으로 잡고, 캐시에서 꺼낸 응답에는 현재
  요청의 질의 ID를 다시 붙입니다. 기본 TTL은 1일이며 `dns_cache_ttl_sec` 또는
  `FREEGSM_DNS_CACHE_TTL_SEC`로 바꿀 수 있습니다. TTL이 만료된 항목은 조회·저장
  시점에 즉시 삭제됩니다. DB 파일은 최초 사용 시 자동 생성되며, `dns_cache_db` /
  `FREEGSM_DNS_CACHE_DB`로 경로를 바꿀 수 있습니다. `dns_cache_ttl_sec: 0`으로
  설정하면 캐시를 완전히 비활성화합니다.
