...
| `dnscache.py` | `DnsCache`: In-memory DNS response cache. `get(query)->bytes\|None` / `put(query, response)`. Cache key strips the 2-byte DNS transaction ID (`query[2:]`) so different IDs for the same question still hit; `_apply_query_id` re-stamps the current ID onto the cached response. TTL enforced at read time (`time.monotonic()`); expired entries pruned on `get`/`put`. `ttl_sec<=0` disables entirely (all ops no-op). Thread-safe via `threading.Lock`. Cache is lost on process exit. |
...
- `FREEGSM_DNS_CACHE_TTL_SEC` (`dns_cache_ttl_sec`) — in-memory DNS cache TTL in
  seconds. Default `86400` (1 day). Set to `0` to disable the cache entirely.
  Cache is lost on process exit.
