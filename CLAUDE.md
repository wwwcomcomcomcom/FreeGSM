...
**macOS port has a different scope (see `docs/MACOS_PORT.md`).** Windows captures
*all* outbound UDP/53 + TCP/53 regardless of destination, so apps with a
hardcoded DNS server are intercepted too. The macOS port instead repoints the
*system* resolver at loopback, so it only covers apps that use the system
resolver. An app that talks straight to a hardcoded plaintext DNS server (e.g.
`8.8.8.8:53`) bypasses DoH. Two further Windows-only features do **not** exist on
macOS: (1) **IPv6** — the macOS resolver binds IPv4 loopback and the tunnel only
redirects IPv4, so IPv6 DNS/HTTPS is uncovered; (2) **the QUIC/HTTP-3 UDP relay**
— with DPI *on*, the `0/1`+`128/1` default-override routes *all* UDP into utun,
but the local SOCKS5 proxy is CONNECT-only (no UDP ASSOCIATE), so tun2socks
can't forward it and QUIC (and other UDP) is dropped. Disable browser HTTP/3 on
macOS; most apps fall back to the TCP path, which *is* fragmented.
